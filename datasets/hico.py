...
# from models.dinov3.data.transforms import make_classification_eval_transform
from datasets.hico_text_label import hico_text_label, hico_unseen_index
import datasets.hico_transforms as T
...
class HICODetection(torch.utils.data.Dataset):
    def __init__(self, img_set, img_folder, anno_file, transforms, cfg):
        ...
        self.unseen_index = hico_unseen_index.get(cfg.ZERO_SHOT.TYPE, [])
        ...
        if img_set == 'train' and len(self.unseen_index) != 0 and cfg.ZERO_SHOT.DEL_UNSEEN:
            tmp = []
        ...
        logger.info("{} contains {} images".format(img_set, len(self.ids)))
    ...
    def set_rare_hois(self, anno_file):
        ...
        if len(self.unseen_index) == 0:
            ...
            logger.info("rare:{}, non-rare:{}".format(len(self.rare_triplets), len(self.non_rare_triplets)))
        else:
            self.rare_triplets = []
            self.non_rare_triplets = []
            for img_anno in annotations:
                ...
                for hoi in hois:
                    ...
                    if hoi['hoi_category_id'] - 1 in self.unseen_index:
                        ...
                        self.non_rare_triplets.append(triplet)
            logger.info("unseen:{}, seen:{}".format(len(self.rare_triplets), len(self.non_rare_triplets)))
...
def build(image_set, cfg):
    ...
    dataset = HICODetection(image_set, img_folder, anno_file,
                            transforms=make_hico_transforms(image_set), cfg=cfg)
    if image_set == 'val':
