...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, cfg):
        self.overlap_iou = 0.5
        self.max_hois = 100

        self.zero_shot_type = cfg.ZERO_SHOT.TYPE

        self.use_nms_filter = cfg.EVAL.USE_NMS_FILTER
        self.thres_nms = cfg.EVAL.THRES_NMS
        self.nms_alpha = cfg.EVAL.NMS_ALPHA
        self.nms_beta = cfg.EVAL.NMS_BETA
        ...
        if self.use_nms_filter:
            logger.info('evaluating with NMS filter...')
        ...
        for i, img_gts in enumerate(gts):
            ...
            img_gts = {k: v.to('cpu').numpy() for k, v in img_gts.items() if k not in ['id', 'filename']}
        ...
        with open(os.path.join(output_dir, cfg.EVAL.JSON_FILE), 'w') as f:
            f.write(json.dumps(str({'preds': self.preds, 'gts': self.gts})))

        logger.info(f"preds: {len(self.preds)}, gts: {len(self.gts)}")
    ...
    def compute_map(self):
        ...
        for triplet in self.gt_triplets:
            ...
            if len(tp) == 0:
                ...
                if orignal_triplet in self.rare_triplets: rare_ap[triplet] = 0
                elif orignal_triplet in self.non_rare_triplets: non_rare_ap[triplet] = 0
                continue

            score = np.array(self.score[triplet])
            sort_inds = np.argsort(-score)
            fp = fp[sort_inds]; tp = tp[sort_inds]
            ...
            if orignal_triplet in self.rare_triplets: rare_ap[triplet] = ap[triplet]
            elif orignal_triplet in self.non_rare_triplets: non_rare_ap[triplet] = ap[triplet]
        ...
        if self.zero_shot_type == "default":
            logger.info(f'mAP full: {m_ap} mAP rare: {m_ap_rare}  mAP non-rare: {m_ap_non_rare}  mean max recall: {m_max_recall}')
            ...
            logger.info(f'mAP full: {m_ap} mAP unseen-obj: {m_ap_rare}  mAP seen-obj: {m_ap_non_rare}  mean max recall: {m_max_recall}')
            return_dict = {'mAP': m_ap, 'mAP unseen-obj': m_ap_rare, 'mAP seen-obj': m_ap_non_rare, 'mean max recall': m_max_recall}
        else:
            logger.info(f'mAP full: {m_ap} mAP unseen: {m_ap_rare}  mAP seen: {m_ap_non_rare}  mean max recall: {m_max_recall}')
    ...
    def compute_iou_mat(self, bbox_list1, bbox_list2):
        ...
        if len(bbox_list1) == 0 or len(bbox_list2) == 0: return {}, {}
        for i, bbox1 in enumerate(bbox_list1):
            for j, bbox2 in enumerate(bbox_list2):
                iou_i = self.compute_IOU(bbox1, bbox2)
                iou_mat[i, j] = iou_i
        iou_mat_ov = iou_mat.copy()
        iou_mat[iou_mat >= self.overlap_iou] = 1; iou_mat[iou_mat < self.overlap_iou] = 0
        match_pairs = np.nonzero(iou_mat); match_pairs_dict = {}; match_pair_overlaps = {}
        if iou_mat.max() > 0:
            for i, pred_id in enumerate(match_pairs[1]):
                if pred_id not in match_pairs_dict.keys():
                    match_pairs_dict[pred_id] = []; match_pair_overlaps[pred_id] = []
    ...
    def compute_IOU(self, bbox1, bbox2):
        rec1 = bbox1['bbox']; rec2 = bbox2['bbox']
        S_rec1 = (rec1[2] - rec1[0] + 1) * (rec1[3] - rec1[1] + 1)
        S_rec2 = (rec2[2] - rec2[0] + 1) * (rec2[3] - rec2[1] + 1)
        sum_area = S_rec1 + S_rec2
        left_line = max(rec1[1], rec2[1]); right_line = min(rec1[3], rec2[3])
        top_line = max(rec1[0], rec2[0]); bottom_line = min(rec1[2], rec2[2])
    ...
    def triplet_nms_filter(self, preds):
        ...
        for img_preds in preds:
            pred_bboxes = img_preds['predictions']; pred_hois = img_preds['hoi_prediction']
            all_triplets = defaultdict(lambda: {'subs': [], 'objs': [], 'scores': [], 'indexes': []})
            ...
            for triplet, values in all_triplets.items():
                ...
                if self.use_soft_nms: keep_inds = self.pairwise_soft_nms(np.array(subs), np.array(objs), np.array(scores))
                else: keep_inds = self.pairwise_nms(np.array(subs), np.array(objs), np.array(scores))
            ...
            preds_filtered.append({
                'filename': img_preds['filename'], 'predictions': pred_bboxes,
    ...
    def pairwise_nms(self, subs, objs, scores):
        ...
        sub_areas = (sx2 - sx1 + 1) * (sy2 - sy1 + 1); obj_areas = (ox2 - ox1 + 1) * (oy2 - oy1 + 1)
        order = scores.argsort()[::-1]; keep_inds = []
        while order.size > 0:
            i = order[0]; keep_inds.append(i)
            sxx1 = np.maximum(sx1[i], sx1[order[1:]]); syy1 = np.maximum(sy1[i], sy1[order[1:]])
            sxx2 = np.minimum(sx2[i], sx2[order[1:]]); syy2 = np.minimum(sy2[i], sy2[order[1:]])
            sw = np.maximum(0.0, sxx2 - sxx1 + 1); sh = np.maximum(0.0, syy2 - syy1 + 1)
            sub_inter = sw * sh; sub_union = sub_areas[i] + sub_areas[order[1:]] - sub_inter
            oxx1 = np.maximum(ox1[i], ox1[order[1:]]); oyy1 = np.maximum(oy1[i], oy1[order[1:]])
            oxx2 = np.minimum(ox2[i], ox2[order[1:]]); oyy2 = np.minimum(oy2[i], oy2[order[1:]])
            ow = np.maximum(0.0, oxx2 - oxx1 + 1); oh = np.maximum(0.0, oyy2 - oyy1 + 1)
            obj_inter = ow * oh; obj_union = obj_areas[i] + obj_areas[order[1:]] - obj_inter
