...
def resize(image, target, size, max_size=None, size_divisor=32):
    ...
    def get_size_with_aspect_ratio(image_size, size, max_size=None):
        ...
        if (w <= h and w == size) or (h <= w and h == size):
            w_mod = np.mod(w, size_divisor)
            h_mod = np.mod(h, size_divisor)
            h = h - h_mod
            w = w - w_mod
            return (h, w)
        ...
        if w < h:
            ow = size
            oh = int(size * h / w)
            ow_mod = np.mod(ow, size_divisor)
            oh_mod = np.mod(oh, size_divisor)
            ow = ow - ow_mod
            oh = oh - oh_mod
        else:
            oh = size
...
class RandomResize(object):
    def __init__(self, sizes, max_size=None, size_divisor=16):
    ...
    def __call__(self, img, target=None):
        ...
        return resize(img, target, size, self.max_size, self.size_divisor)
