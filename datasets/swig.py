...
import datasets.swig_transforms as T
...
def make_transforms(image_set, eval_size=512):
    normalize = T.Compose([
        T.ToTensor(),
    ...
    if image_set == "val":
        return T.Compose([
            T.RandomResize([eval_size], max_size=eval_size*1333//800),
            normalize
        ])
...
def build(image_set, cfg):
    ...
    PATHS = {
        'train': (root / 'images_512', root / 'annotations' / 'swig_trainval_1000.json'),
        'val': (root / 'images_512', root / 'annotations' / 'swig_test_1000.json'),
        'dev': (root / 'images_512', root / 'annotations' / 'swig_test_1000.json'),
    }
    ...
    dataset = SWiGHOIDetection(
        img_folder,
        ann_file,
        transforms=make_transforms(image_set, cfg.INPUT.EVAL_SIZE),
        image_set=image_set,
        repeat_factor_sampling=cfg.INPUT.REPEAT_FACTOR_SAMPLING,
    )
